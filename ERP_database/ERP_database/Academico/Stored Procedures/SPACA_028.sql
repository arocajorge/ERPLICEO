...
SELECT cp.IdEmpresa, cp.IdAnio, a.Descripcion, cp.IdSede, s.NomSede, cp.IdNivel, n.NomNivel, cp.IdJornada, j.NomJornada, cp.IdCurso, c.NomCurso, cp.IdParalelo, cp.CodigoParalelo, cp.NomParalelo, cp.OrdenParalelo, cp.IdProfesorTutor, 
                  ...
                  c.OrdenCurso
FROM     dbo.tb_persona AS pi with (nolock) INNER JOIN
                  dbo.aca_Profesor AS insp with (nolock) ON pi.IdPersona = insp.IdPersona RIGHT OUTER JOIN
                  dbo.aca_AnioLectivo_Curso_Paralelo AS cp INNER JOIN
                  dbo.aca_AnioLectivo AS a with (nolock) ON cp.IdEmpresa = a.IdEmpresa AND cp.IdAnio = a.IdAnio INNER JOIN
                  ...
                  dbo.aca_NivelAcademico AS n with (nolock) ON cp.IdEmpresa = n.IdEmpresa AND cp.IdNivel = n.IdNivel ON insp.IdEmpresa = cp.IdEmpresa AND insp.IdProfesor = cp.IdProfesorInspector LEFT OUTER JOIN
                  dbo.tb_persona AS pt with (nolock) INNER JOIN
                  dbo.aca_Profesor AS tut with (nolock) ON pt.IdPersona = tut.IdPersona ON cp.IdEmpresa = tut.IdEmpresa AND cp.IdProfesorTutor = tut.IdProfesor
