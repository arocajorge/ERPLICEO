...
FROM     dbo.aca_MatriculaCalificacion AS mc with (nolock) inner JOIN
				  ...
				  LEFT OUTER JOIN aca_Profesor pro with (nolock) on pro.IdEmpresa=mc.IdEmpresa and pro.IdProfesor=mc.IdProfesor
				  inner join tb_persona pp with (nolock) on pp.IdPersona=pro.IdPersona
				  INNER JOIN
...
(
	...
	SELECT mc.IdEmpresa, mc.IdMateria, mc.IdProfesor, c.IdAnio, c.IdSede, c.IdNivel, c.IdJornada, c.IdCurso, c.IdParalelo, a.Descripcion, sn.NomSede, sn.NomNivel, sn.OrdenNivel, nj.NomJornada, nj.OrdenJornada, jc.NomCurso, 
		...
		null as EXQuim2
	FROM     dbo.aca_MatriculaCalificacionCualitativaPromedio AS mc with (nolock) inner JOIN
		dbo.aca_Matricula AS c with (nolock) ON mc.IdEmpresa = c.IdEmpresa AND mc.IdMatricula = c.IdMatricula INNER JOIN
		dbo.aca_AnioLectivo AS a with (nolock) ON c.IdAnio = a.IdAnio AND c.IdEmpresa = a.IdEmpresa 
		LEFT OUTER JOIN aca_Profesor pro with (nolock) on pro.IdEmpresa=mc.IdEmpresa and pro.IdProfesor=mc.IdProfesor
		inner join tb_persona pp with (nolock) on pp.IdPersona=pro.IdPersona
