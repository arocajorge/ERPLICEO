CREATE VIEW Academico.VWACA_021
AS
SELECT m.IdEmpresa, m.IdMatricula, al.Codigo, m.IdAlumno, pa.IdPersona, pa.pe_nombreCompleto AS NombreAlumno, pa.pe_cedulaRuc AS CedulaAlumno, m.IdAnio, m.IdSede, m.IdNivel, m.IdJornada, m.IdCurso, m.IdParalelo, sn.NomSede, 
                  ...
                  END AS EsRetirado, m.IdPlantilla, pl.NomPlantilla, pt.NomPlantillaTipo
FROM     dbo.aca_Matricula AS m WITH (nolock) INNER JOIN
                  ...
                  dbo.aca_Plantilla AS pl WITH (nolock) ON m.IdEmpresa = pl.IdEmpresa AND m.IdAnio = pl.IdAnio AND m.IdPlantilla = pl.IdPlantilla INNER JOIN
...
WHERE  (al.Estado = 1)
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 2, @level0type = N'SCHEMA', @level0name = N'Academico', @level1type = N'VIEW', @level1name = N'VWACA_021';
...
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPane2', @value = N'     End
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'Academico', @level1type = N'VIEW', @level1name = N'VWACA_021';
