...
SELECT cabfac.IdEmpresa, cabfac.IdSucursal, cabfac.IdBodega, cabfac.vt_tipoDoc, cabfac.vt_tipoDoc + '-' + CAST(CAST(cabfac.vt_NumFactura AS int) AS varchar(20)) AS vt_NunDocumento, cabfac.vt_Observacion AS Referencia, 
				  ...
				  CASE WHEN aca_AnioLectivo_Rubro.AplicaProntoPago = 1 AND aca_AnioLectivo_Periodo.FechaProntoPago >= CAST(GETDATE() AS DATE) 
                  THEN detfac.ValorProntoPago - round(isnull(vwcxc_total_cobros_x_Docu.dc_ValorPago,0),2) ELSE ROUND(detfac.Total - ROUND(ISNULL(vwcxc_total_cobros_x_Docu.dc_ValorPago, 0), 2), 2) END AS SaldoProntoPago
...
SELECT A.IdEmpresa, A.IdSucursal, A.IdBodega, 'NTDB' AS CreDeb, CASE WHEN A.NumNota_Impresa IS NULL THEN 'N/D#' + CAST(A.IdNota AS varchar(20)) ELSE 'N/D#' + A.Serie1 + '-' + A.Serie2 + '' + A.NumNota_Impresa END AS Documento, 
                  ...
                  Cli.Codigo AS CodCliente, tb_persona.pe_nombreCompleto, tb_empresa.em_nombre, A.Estado, ROUND(SUM(B.sc_total), 2), NULL, NULL IdAnio, NULL IdPlantilla, A.IdPuntoVta, ROUND(SUM(B.sc_total) - ISNULL(SUM(CB.dc_ValorPago), 0), 2) SaldoProntoPago
