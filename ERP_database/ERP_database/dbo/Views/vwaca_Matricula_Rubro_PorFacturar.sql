...
AS
SELECT        dbo.aca_Matricula_Rubro.IdEmpresa, dbo.aca_Matricula_Rubro.IdMatricula, dbo.aca_Matricula_Rubro.IdPeriodo, dbo.aca_Matricula_Rubro.IdRubro, dbo.aca_Matricula_Rubro.IdMecanismo, 
                         dbo.aca_Matricula_Rubro.IdProducto, dbo.aca_Matricula_Rubro.EnMatricula, CAST(dbo.aca_Matricula_Rubro.Subtotal AS float) AS Subtotal, dbo.aca_Matricula_Rubro.IdCod_Impuesto_Iva, 
                         CAST(dbo.aca_Matricula_Rubro.Porcentaje AS float) AS Porcentaje, CAST(dbo.aca_Matricula_Rubro.ValorIVA AS float) AS ValorIVA, CAST(dbo.aca_Matricula_Rubro.Total AS float) AS Total, dbo.aca_Matricula_Rubro.IdSucursal, 
                         dbo.aca_Matricula_Rubro.IdBodega, dbo.aca_Matricula_Rubro.IdCbteVta, dbo.aca_Matricula_Rubro.FechaFacturacion, dbo.aca_Matricula.IdAnio, dbo.aca_Matricula.IdSede, dbo.aca_Matricula.IdAlumno, 
                         dbo.aca_AnioLectivo_Rubro.AplicaProntoPago, dbo.aca_AnioLectivo_Rubro.NomRubro, dbo.in_Producto.pr_descripcion, dbo.aca_AnioLectivo_Periodo.FechaDesde, 
                         CASE WHEN dbo.aca_AnioLectivo_Rubro.NumeroCuotas = 1 THEN '' ELSE CAST(dbo.aca_AnioLectivo_Rubro_Periodo.Secuencia AS varchar) + '/' + CAST(dbo.aca_AnioLectivo_Rubro.NumeroCuotas AS varchar) 
                         END AS DescripcionCuotas, CASE WHEN dbo.aca_AnioLectivo_Periodo.FechaProntoPago > CAST(getdate() AS date) THEN CAST(dbo.aca_Matricula_Rubro.Subtotal AS float) - case when dbo.aca_Plantilla.TipoDescuento = '$' THEN CAST(dbo.aca_Matricula_Rubro.Subtotal AS float) - CAST(dbo.aca_Plantilla.Valor AS FLOAT) ELSE ROUND(CAST(dbo.aca_Matricula_Rubro.Subtotal AS float) * (dbo.aca_Plantilla.Valor/100),2) END ELSE CAST(dbo.aca_Matricula_Rubro.Total AS float) + CAST(dbo.aca_Matricula_Rubro.ValorIVA AS FLOAT)
                         END AS ValorProntoPago, dbo.aca_AnioLectivo_Periodo.FechaProntoPago
