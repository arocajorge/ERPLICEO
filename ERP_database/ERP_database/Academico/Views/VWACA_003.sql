...
SELECT m.IdEmpresa, m.IdMatricula, m.IdAlumno, Representante.pe_nombreCompleto AS NomRepresentante, Representante.pe_cedulaRuc AS CedulaRepresentante, Factura.pe_nombreCompleto AS NomSeFactura, 
                  ...
                  AS DescripcionPensiones, FFactura.Correo AS CorreoSeFactura, FRepresentante.Correo AS CorreoRepresentante
FROM     dbo.tb_persona AS p INNER JOIN
                  ...
                  dbo.tb_mes AS MesFin ON MONTH(AnioActual.FechaHasta) = MesFin.idMes LEFT OUTER JOIN
                  dbo.aca_Familia AS FFactura ON m.IdEmpresa = FFactura.IdEmpresa AND m.IdAlumno = FFactura.IdAlumno AND m.IdPersonaR = FFactura.IdPersona LEFT OUTER JOIN
                  dbo.aca_Familia AS FRepresentante ON m.IdPersonaR = FRepresentante.IdPersona AND m.IdEmpresa = FRepresentante.IdEmpresa AND m.IdAlumno = FRepresentante.IdAlumno
GO
      ...
      Begin Tables = 
         ...
         Begin Table = "aca_AnioLectivo_Jornada_Curso"
            Begin Extent = 
               ...
               Right = 242
            End
            Displ', @level0type = N'SCHEMA', @level0name = N'Academico', @level1type = N'VIEW', @level1name = N'VWACA_003';
