...
,1						,'Sueldo base'	, emp.Tiene_ingresos_compartidos,emp.Pago_por_horas	
FROM            dbo.ro_contrato AS cont INNER JOIN
                dbo.ro_empleado AS emp ON cont.IdEmpresa = emp.IdEmpresa AND cont.IdEmpleado = emp.IdEmpleado
where cont.IdEmpresa=@IdEmpresa 
and cont.IdNomina=@IdNomina
...
,1						,rub.ru_descripcion +'('+ CAST(  convert(decimal(10, 2), (SUM(ISNULL( nov.CantidadHoras,0)))) as varchar(10))   +')' , emp.Tiene_ingresos_compartidos,emp.Pago_por_horas			
...
,1						,rub.ru_descripcion, emp.Tiene_ingresos_compartidos,emp.Pago_por_horas	
...
,1						,'Aporte personal'	, emp.Tiene_ingresos_compartidos,emp.Pago_por_horas	
...
,1						,'Anticipo 25 primera quincena'	, emp.Tiene_ingresos_compartidos,emp.Pago_por_horas	
FROM            dbo.ro_rol_detalle AS rol_det INNER JOIN
                         dbo.ro_rubro_tipo AS rub ON rol_det.IdEmpresa = rub.IdEmpresa AND rol_det.IdRubro = rub.IdRubro INNER JOIN
                         dbo.ro_rol ON rol_det.IdEmpresa = dbo.ro_rol.IdEmpresa AND rol_det.IdRol = dbo.ro_rol.IdRol INNER JOIN
                         dbo.ro_empleado AS emp ON rol_det.IdEmpresa = emp.IdEmpresa AND rol_det.IdEmpleado = emp.IdEmpleado INNER JOIN
...
,1						,'Fondos de reserva', emp.Tiene_ingresos_compartidos,emp.Pago_por_horas		
FROM            dbo.ro_rol_detalle AS rol_det INNER JOIN
                         dbo.ro_rubro_tipo AS rub ON rol_det.IdEmpresa = rub.IdEmpresa AND rol_det.IdRubro = rub.IdRubro INNER JOIN
                         dbo.ro_rol ON rol_det.IdEmpresa = dbo.ro_rol.IdEmpresa AND rol_det.IdRol = dbo.ro_rol.IdRol INNER JOIN
...
,1						,'Decimo tercer sueldo'	, emp.Tiene_ingresos_compartidos,emp.Pago_por_horas	
FROM            dbo.ro_rol_detalle AS rol_det INNER JOIN
                         dbo.ro_rubro_tipo AS rub ON rol_det.IdEmpresa = rub.IdEmpresa AND rol_det.IdRubro = rub.IdRubro INNER JOIN
                         dbo.ro_rol ON rol_det.IdEmpresa = dbo.ro_rol.IdEmpresa AND rol_det.IdRol = dbo.ro_rol.IdRol INNER JOIN
                         dbo.ro_empleado AS emp ON rol_det.IdEmpresa = emp.IdEmpresa AND rol_det.IdEmpleado = emp.IdEmpleado INNER JOIN
...
dbo.calcular_dias_trabajados(@Fi,@Ff,emp.em_fechaIngaRol, emp.em_status, emp.em_fechaSalida),2) 
,1						,'Decimo cuarto sueldo'	, emp.Tiene_ingresos_compartidos,emp.Pago_por_horas	
...
,1						,'Total ingresos', emp.Tiene_ingresos_compartidos,emp.Pago_por_horas		
FROM            dbo.ro_rol_detalle AS rol_det INNER JOIN
                         dbo.ro_rubro_tipo AS rub ON rol_det.IdEmpresa = rub.IdEmpresa AND rol_det.IdRubro = rub.IdRubro INNER JOIN
                         dbo.ro_rol ON rol_det.IdEmpresa = dbo.ro_rol.IdEmpresa AND rol_det.IdRol = dbo.ro_rol.IdRol INNER JOIN
                         dbo.ro_empleado AS emp ON rol_det.IdEmpresa = emp.IdEmpresa AND rol_det.IdEmpleado = emp.IdEmpleado INNER JOIN
...
,1						,'Total Egreso'	, emp.Tiene_ingresos_compartidos,emp.Pago_por_horas	
FROM            dbo.ro_rol_detalle AS rol_det INNER JOIN
                         dbo.ro_rubro_tipo AS rub ON rol_det.IdEmpresa = rub.IdEmpresa AND rol_det.IdRubro = rub.IdRubro INNER JOIN
                         dbo.ro_rol ON rol_det.IdEmpresa = dbo.ro_rol.IdEmpresa AND rol_det.IdRol = dbo.ro_rol.IdRol INNER JOIN
                         dbo.ro_empleado AS emp ON rol_det.IdEmpresa = emp.IdEmpresa AND rol_det.IdEmpleado = emp.IdEmpleado INNER JOIN
...
--CASE WHEN emp.Pago_por_horas = 1 then 
ISNULL( round( (sum(isnull(rol_det.Valor,0)) /30) * dbo.calcular_dias_fondos_reserva(@Fi,@Ff,emp.em_fechaIngaRol, emp.em_status, ISNULL(EMP.em_fechaSalida,DATEADD(YEAR,50,GETDATE())), contI.FechaAcumulacion)*0.0833 ,2),0)
--else ISNULL(round( sum(rol_det.Valor)*0.0833 ,2),0) end
...
select
@IdEmpresa				,@IdRol				,emp.IdSucursal				,rol_det.IdEmpleado		,@IdRubro_calculado				,round((sum(isnull(rol_det.Valor,0))/360)*30,2)
...
select
@IdEmpresa				,@IdRol				,emp.IdSucursal			,emp.IdEmpleado		,@IdRubro_calculado				,round((isnull(@SueldoBasico,0)/360)* 
...
select IdEmpresa,				IdRol,			IdSucursal,						IdEmpleado,			IdRubro, IdJornada,			Orden,		ROUND(SUM(isnull(Valor,0)),2)
...
SELECT IdEmpresa,				IdRol,			IdSucursal,						IdEmpleado,			IdRubro,IdJornada ,			Orden,			ROUND(sum(isnull(Valor,0)),2)Valor
...
select IdEmpresa,				IdRol,			IdSucursal,			IdEmpleado,			IdRubro,IdJornada,			Orden,			sum(isnull(Valor,0))Valor
,rub_visible_reporte,	Observacion from
(
select
...
,rub_visible_reporte,	Observacion)
select
@IdEmpresa				,@IdRol				,emp.IdSucursal						,emp.IdEmpleado		,@IdRubro_calculado,1	,'51'			,ROUND((isnull(@SueldoBasico,0)/360)* 
...
,rub_visible_reporte,	Observacion)
select
@IdEmpresa				,@IdRol				,emp.IdSucursal						,emp.IdEmpleado		,@IdRubro_calculado,2	,'51'			,ROUND((isnull(@SueldoBasico,0)/360)* 
dbo.calcular_dias_trabajados(@Fi,@Ff,emp.em_fechaIngaRol, emp.em_status, emp.em_fechaSalida) *0.5,2)
,1						,'Decimo cuarto sueldo'	
FROM  dbo.ro_empleado emp, ro_contrato cont
where emp.IdEmpresa=cont.IdEmpresa
...
----------------------------------------------------------------------------------------------------------------------------------------------
-------------PROVISION DECIMO TERCERO POR JORNADA-----------------------------------------------------------------------------------------------------<
----------------------------------------------------------------------------------------------------------------------------------------------
...
select 
@IdEmpresa	IdEmpresa			,@IdRol	IdRol			,emp.IdSucursal							,novc.IdEmpleado		,@IdRubro_calculado IdRubro, ISNULL( novc.IdJornada,1)IdJornada		,0 Orden,   sum(nov.Valor)/12 Valor
,1	rub_visible_reporte					,'PROVISION DECIMO TERCER SUELDO' Observacion		
FROM   dbo.ro_empleado AS emp INNER JOIN
dbo.ro_empleado_Novedad AS novc ON emp.IdEmpresa = novc.IdEmpresa AND emp.IdEmpleado = novc.IdEmpleado INNER JOIN
dbo.ro_empleado_novedad_det AS nov ON novc.IdEmpresa = nov.IdEmpresa AND novc.IdNovedad = nov.IdNovedad AND novc.IdEmpleado = novc.IdEmpleado INNER JOIN
dbo.ro_rubro_tipo AS rub ON nov.IdEmpresa = rub.IdEmpresa AND nov.IdRubro = rub.IdRubro
...
----------------------------------------------------------------------------------------------------------------------------------------------
-------------PROVISION FONDO DE RESERVA POR JORNADA----------------------------------------------------------------------------------------------------<
----------------------------------------------------------------------------------------------------------------------------------------------
select @IdRubro_calculado= IdRubro_prov_FR,@IdRubro_Provision=IdRubro_fondo_reserva from ro_rubros_calculados where IdEmpresa=@IdEmpresa-- obteniendo el idrubro desde parametros
insert into ro_rol_detalle_x_jornada
(IdEmpresa,				IdRol,			IdSucursal,			IdEmpleado,			IdRubro,IdJornada,			Orden,			Valor
...
,1	rub_visible_reporte					,'Provision Fondos de reserva' Observacion	
FROM            dbo.ro_empleado AS emp INNER JOIN
                         dbo.ro_empleado_Novedad AS novc ON emp.IdEmpresa = novc.IdEmpresa AND emp.IdEmpleado = novc.IdEmpleado INNER JOIN
                         dbo.ro_empleado_novedad_det AS nov ON novc.IdEmpresa = nov.IdEmpresa AND novc.IdNovedad = nov.IdNovedad INNER JOIN
                         dbo.ro_rubro_tipo AS rub ON nov.IdEmpresa = rub.IdEmpresa AND nov.IdRubro = rub.IdRubro
...
----------------------------------------------------------------------------------------------------------------------------------------------
------------- PROVISION DECIMO CUARTO POR JORNADA UNO-----------------------------------------------------------------------------------------------------<
----------------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------------
select @IdRubro_calculado= IdRubro_prov_DIV, @IdRubro_Provision=IdRubro_DIV from ro_rubros_calculados where IdEmpresa=@IdEmpresa-- obteniendo el idrubro desde parametros
insert into ro_rol_detalle_x_jornada
...
select
@IdEmpresa				,@IdRol				,emp.IdSucursal						,emp.IdEmpleado		,@IdRubro_calculado,1	,'51'			,ROUND((isnull(@SueldoBasico,0)/360)* 
isnull(dbo.calcular_dias_trabajados(@Fi,@Ff,emp.em_fechaIngaRol, emp.em_status, emp.em_fechaSalida),0) *0.5,2)
...
----------------------------------------------------------------------------------------------------------------------------------------------
------------- PROVISION DECIMO CUARTO POR JORNADA DOS-----------------------------------------------------------------------------------------------------<
----------------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------------
select @IdRubro_calculado= IdRubro_prov_DIV, @IdRubro_Provision=IdRubro_DIV from ro_rubros_calculados where IdEmpresa=@IdEmpresa-- obteniendo el idrubro desde parametros
insert into ro_rol_detalle_x_jornada
