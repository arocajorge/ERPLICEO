...
SELECT a.IdEmpresa, A.IdRol, A.IdEmpleado, A.IdDivision, A.IdArea, A.IdDepartamento, A.IdSucursal,IdNominaTipo,  A.Descripcion,  A.IdNominaTipoLiqui,DescripcionProcesoNomina,  A.IdPeriodo, A.pe_nombreCompleto, A.NombreDivision, A.NombreArea, A.NombreDepartamento, A.Su_Descripcion,  ( A.SUELDO)*(B.Porcentaje/100) SUELDO, case when B.CargaGasto=1 then  A.ANTICIPO else 0 end ANTICIPO,  (A.DECIMOC)*(B.Porcentaje/100) DECIMOC, ( A.DECIMOT)*(B.Porcentaje/100) DECIMOT, 
  ( (A.FRESERVA - t.ValorConDivision) * 0.0833)*(B.Porcentaje/100) FRESERVA, 
 ( (A.IESS  - t.ValorConDivision) * 0.0945)*(B.Porcentaje/100) IESS, 

case when B.CargaGasto=1 then  A.PRESTAMO else 0 end PRESTAMO,( A.SOBRET)*(B.Porcentaje/100) SOBRET,
 case when B.CargaGasto=1 then A.TOTALE else 0 end TOTALE, ( A.TOTALI)*(B.Porcentaje/100)TOTALI, case when B.CargaGasto=1 then  A.OTROEGR else 0 end OTROEGR,  (A.OTROING)*(B.Porcentaje/100)OTROING,  A.DIASTRABAJADOS, case when B.CargaGasto=1 then  ((A.TOTALI)*(B.Porcentaje/100)) - a.TOTALE else ( A.TOTALI)*(B.Porcentaje/100) end NETO,NombreCargo, A.JORNADA, B.Descripcion AS UBUCACION, B.Porcentaje,
...
ro_cargo.ca_descripcion as NombreCargo,
				  ro_rubro_tipo.ru_tipo,ro_catalogo.ca_descripcion as NombreRubro,
				  CASE WHEN ro_catalogo.CodCatalogo = 'SUELDO' THEN  dbo.ro_rol_detalle.VALOR  ELSE 0 END AS SUELDO,
				  CASE WHEN ro_catalogo.CodCatalogo = 'ANTICIPO' THEN dbo.ro_rol_detalle.VALOR ELSE 0 END AS ANTICIPO,
				  CASE WHEN ro_catalogo.CodCatalogo = 'DECIMOC' THEN  dbo.ro_rol_detalle.VALOR ELSE 0 END AS DECIMOC,
				  CASE WHEN ro_catalogo.CodCatalogo = 'DECIMOT' THEN  dbo.ro_rol_detalle.VALOR ELSE 0 END AS DECIMOT,
				  case when ( select SUM( d.Valor) from ro_rol_detalle d 
				  where  d.IdEmpresa=ro_rol_detalle.IdEmpresa 
				  AND d.IdRol=ro_rol.IdRol 
				  and ro_rol_detalle.IdEmpleado=d.IdEmpleado 
				  and d.IdRubro=ro_rubros_calculados.IdRubro_fondo_reserva AND d.Valor>0) is not null then  CASE WHEN ro_rubro_tipo.rub_aplica_IESS=1 THEN  dbo.ro_rol_detalle.VALOR/* *(0.0833) */ ELSE 0 END ELSE 0 END AS FRESERVA,
				  CASE WHEN ro_contrato.IdNomina=1 THEN CASE WHEN ro_rubro_tipo.rub_aplica_IESS = 1 THEN  dbo.ro_rol_detalle.VALOR /**(ro_rol.PorAportePersonal)*/ ELSE 0 END ELSE 0 END AS IESS,
				  CASE WHEN ro_catalogo.CodCatalogo = 'PRESTAMO' THEN dbo.ro_rol_detalle.VALOR ELSE 0 END AS PRESTAMO,
				  CASE WHEN ro_catalogo.CodCatalogo = 'SOBRET' THEN  dbo.ro_rol_detalle.VALOR ELSE 0 END AS SOBRET,
				  CASE WHEN ro_rubro_tipo.ru_tipo = 'E' THEN dbo.ro_rol_detalle.VALOR ELSE 0 END AS TOTALE,
				  CASE WHEN ro_rubro_tipo.ru_tipo = 'I' THEN dbo.ro_rol_detalle.VALOR ELSE 0 END AS TOTALI,
				  CASE WHEN ro_rubro_tipo.ru_tipo = 'E' AND ro_rubro_tipo.rub_GrupoResumen is null THEN dbo.ro_rol_detalle.VALOR ELSE 0 END AS OTROEGR,
				  CASE WHEN ro_rubro_tipo.ru_tipo = 'I' AND ro_rubro_tipo.rub_GrupoResumen is null AND ROLF.IdDivision IS NULL THEN dbo.ro_rol_detalle.VALOR ELSE 0 END AS OTROING,
				  CASE WHEN ro_rubro_tipo.IdRubro = ro_rubros_calculados.IdRubro_dias_trabajados then dbo.ro_rol_detalle.VALOR ELSE 0 END AS DIASTRABAJADOS,				  
				  ' ' JORNADA
...
FROM            ro_empleado_division_area_x_rol AS emp_x_are_xrol INNER JOIN
                         ...
                         ro_empleado_x_division_x_area ON area.IdEmpresa = ro_empleado_x_division_x_area.IdEmpresa AND area.IdDivision = ro_empleado_x_division_x_area.IDividion AND 
                         area.IdArea = ro_empleado_x_division_x_area.IdArea AND emp_x_are_xrol.IdEmpleado = ro_empleado_x_division_x_area.IdEmpleado
where emp_x_are_xrol.Porcentaje != 0
				  ) B ON 
				  A.IdEmpresa=B.IdEmpresa
				  AND A.IdRol=B.IdRol
				  AND A.IdEmpleado=B.IdEmpleado
				  LEFT JOIN(
					SELECT G.IdEmpresa, G.IdRol, G.IdEmpleado, SUM(G.IESS_TOTAL)IESS_TOTAL, SUM(G.FRESERVA_TOTAL) FRESERVA_TOTAL, isnull(ValorConDivision,0)ValorConDivision
					FROM (
					SELECT D.IdEmpresa, D.IdRol, D.IdEmpleado,
					CASE WHEN CA.CodCatalogo = 'IESS' THEN D.Valor ELSE 0 END AS IESS_TOTAL,
					CASE WHEN CA.CodCatalogo = 'FRESERVA' THEN D.Valor ELSE 0 END AS FRESERVA_TOTAL,
					...
					INNER JOIN dbo.ro_catalogo CA ON R.rub_GrupoResumen = CA.CodCatalogo 
					left join (
						select a1.IdEmpresa, a1.IdRol, a1.IdEmpleado, sum(a1.Valor) as ValorConDivision
						from ro_rol_detalle_x_ro_rubro_fijo as a1 inner join ro_rubro_tipo as a2
						on a1.IdEmpresa = a2.IdEmpresa and a1.IdRubro = a2.IdRubro
						where a1.IdDivision is not null AND A2.rub_aplica_IESS = 1
						group by a1.IdEmpresa, a1.IdRol, a1.IdEmpleado
					) RE
					ON D.IdEmpresa = RE.IdEmpresa AND D.IdRol = RE.IdRol AND D.IdEmpleado = RE.IdEmpleado
					WHERE D.IdEmpresa = @IdEmpresa		
					and D.IdSucursal>=@IdSucursalIni
					and D.IdSucursal<=@IdSucursalFin
					and C.IdNominaTipo=@IdNomina
					...
					GROUP BY G.IdEmpresa, G.IdRol, G.IdEmpleado,ValorConDivision
				)
...
( A.FRESERVA) FRESERVA, 
( A.IESS) IESS, 
case when B.CargaGasto=1 then  A.PRESTAMO else 0 end PRESTAMO,( A.SOBRET) SOBRET,
 case when B.CargaGasto=1 then A.TOTALE else 0 end TOTALE, ( A.TOTALI) TOTALI, case when B.CargaGasto=1 then  A.OTROEGR else 0 end OTROEGR,  (A.OTROING)OTROING,  A.DIASTRABAJADOS, case when B.CargaGasto=1 then  (A.TOTALI) - a.TOTALE else ( A.TOTALI) end NETO,NombreCargo, A.JORNADA, B.Descripcion AS UBUCACION, B.Porcentaje,
 T.IESS_TOTAL, T.FRESERVA_TOTAL
...
SUM(TOTALE)TOTALE, SUM(TOTALI)TOTALI,SUM(OTROEGR)OTROEGR, SUM(OTROING) OTROING, SUM(DIASTRABAJADOS) DIASTRABAJADOS, (SUM(TOTALI) - SUM(TOTALE)) NETO,NombreCargo, JORNADA
FROM (
SELECT ro_rol_detalle_x_ro_rubro_fijo.IdEmpresa, ro_rol_detalle_x_ro_rubro_fijo.IdRol, ro_rol_detalle_x_ro_rubro_fijo.IdEmpleado, ro_empleado.IdDivision, ro_empleado.IdArea, ro_empleado.IdDepartamento, ro_rol.IdSucursal, ro_rol.IdNominaTipo,ro_Nomina_Tipo.Descripcion, ro_rol.IdNominaTipoLiqui, ro_Nomina_Tipoliqui.DescripcionProcesoNomina, tb_sucursal.Su_Descripcion,
ro_rol.IdPeriodo, tb_persona.pe_nombreCompleto, ro_Division.Descripcion AS NombreDivision, ro_area.Descripcion AS NombreArea, ro_Departamento.de_descripcion AS NombreDepartamento,
ro_cargo.ca_descripcion as NombreCargo,
				  ro_rubro_tipo.ru_tipo,ro_catalogo.ca_descripcion as NombreRubro,
				  CASE WHEN ro_catalogo.CodCatalogo = 'SUELDO' THEN  VALOR  ELSE 0 END AS SUELDO,
				  ...
				  CASE WHEN ro_catalogo.CodCatalogo = 'DECIMOC' THEN  VALOR ELSE 0 END AS DECIMOC,
				  CASE WHEN ro_catalogo.CodCatalogo = 'DECIMOT' THEN  VALOR ELSE 0 END AS DECIMOT,
				  case when ( select SUM( D.Valor) from ro_rol_detalle d where  d.IdEmpresa=ro_rol_detalle_x_ro_rubro_fijo.IdEmpresa AND d.IdRol=ro_rol.IdRol and ro_rol_detalle_x_ro_rubro_fijo.IdEmpleado=d.IdEmpleado and d.IdRubro=ro_rubros_calculados.IdRubro_fondo_reserva AND d.Valor>0) is not null then  CASE WHEN ro_rubro_tipo.rub_aplica_IESS=1 THEN  VALOR*(0.0833)  ELSE 0 END ELSE 0 END AS FRESERVA,
				  CASE WHEN ro_contrato.IdNomina=1 THEN CASE WHEN ro_rubro_tipo.rub_aplica_IESS = 1 THEN  VALOR*(ro_rol.PorAportePersonal) ELSE 0 END ELSE 0 END AS IESS,
				  CASE WHEN ro_catalogo.CodCatalogo = 'PRESTAMO' THEN VALOR ELSE 0 END AS PRESTAMO,
				  CASE WHEN ro_catalogo.CodCatalogo = 'SOBRET' THEN  VALOR ELSE 0 END AS SOBRET,
				  CASE WHEN ro_rubro_tipo.ru_tipo = 'E' THEN VALOR ELSE 0 END AS TOTALE,
...
FROM            dbo.tb_persona INNER JOIN
                         ...
                         dbo.ro_Departamento ON dbo.ro_empleado.IdEmpresa = dbo.ro_Departamento.IdEmpresa AND dbo.ro_empleado.IdDepartamento = dbo.ro_Departamento.IdDepartamento RIGHT OUTER JOIN
                         dbo.ro_rol INNER JOIN
                         dbo.ro_rol_detalle_x_ro_rubro_fijo ON dbo.ro_rol.IdEmpresa = dbo.ro_rol_detalle_x_ro_rubro_fijo.IdEmpresa AND dbo.ro_rol.IdRol = dbo.ro_rol_detalle_x_ro_rubro_fijo.IdRol INNER JOIN
                         dbo.ro_rubro_tipo ON dbo.ro_rol_detalle_x_ro_rubro_fijo.IdEmpresa = dbo.ro_rubro_tipo.IdEmpresa AND dbo.ro_rol_detalle_x_ro_rubro_fijo.IdRubro = dbo.ro_rubro_tipo.IdRubro INNER JOIN
                         dbo.ro_rubros_calculados ON dbo.ro_rol_detalle_x_ro_rubro_fijo.IdEmpresa = dbo.ro_rubros_calculados.IdEmpresa ON dbo.ro_empleado.IdEmpresa = dbo.ro_rol_detalle_x_ro_rubro_fijo.IdEmpresa AND 
                         dbo.ro_empleado.IdEmpleado = dbo.ro_rol_detalle_x_ro_rubro_fijo.IdEmpleado LEFT OUTER JOIN
                         dbo.ro_Nomina_Tipo INNER JOIN
                         dbo.ro_Nomina_Tipoliqui ON dbo.ro_Nomina_Tipo.IdEmpresa = dbo.ro_Nomina_Tipoliqui.IdEmpresa AND dbo.ro_Nomina_Tipo.IdNomina_Tipo = dbo.ro_Nomina_Tipoliqui.IdNomina_Tipo ON 
                         dbo.ro_rol.IdEmpresa = dbo.ro_Nomina_Tipoliqui.IdEmpresa AND dbo.ro_rol.IdNominaTipo = dbo.ro_Nomina_Tipoliqui.IdNomina_Tipo AND 
                         dbo.ro_rol.IdNominaTipoLiqui = dbo.ro_Nomina_Tipoliqui.IdNomina_TipoLiqui LEFT OUTER JOIN
