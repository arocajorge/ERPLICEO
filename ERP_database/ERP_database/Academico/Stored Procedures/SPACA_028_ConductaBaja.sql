...
FROM     dbo.aca_MatriculaConducta AS mco INNER JOIN
                  ...
                  dbo.aca_Alumno AS al ON mat.IdEmpresa = al.IdEmpresa AND mat.IdAlumno = al.IdAlumno INNER JOIN
                  dbo.tb_persona AS p ON al.IdPersona = p.IdPersona LEFT OUTER JOIN
                  dbo.aca_Matricula AS m ON al.IdAlumno = m.IdAlumno AND al.IdEmpresa = m.IdEmpresa AND mco.IdEmpresa = m.IdEmpresa AND mco.IdMatricula = m.IdMatricula LEFT OUTER JOIN
                  dbo.aca_AnioLectivoConductaEquivalencia AS equivq1 ON equivq1.IdEmpresa = mco.IdEmpresa AND equivq1.IdAnio = m.IdAnio AND equivq1.Secuencia = mco.SecuenciaPromedioFinalQ1 LEFT OUTER JOIN
                  dbo.aca_AnioLectivoConductaEquivalencia AS equivq2 ON equivq2.IdEmpresa = mco.IdEmpresa AND equivq2.IdAnio = m.IdAnio AND equivq2.Secuencia = mco.SecuenciaPromedioFinalQ2 LEFT OUTER JOIN
                  dbo.aca_AnioLectivoConductaEquivalencia AS equivf ON equivf.IdEmpresa = mco.IdEmpresa AND equivf.IdAnio = m.IdAnio AND equivf.Secuencia = mco.SecuenciaPromedioFinal
