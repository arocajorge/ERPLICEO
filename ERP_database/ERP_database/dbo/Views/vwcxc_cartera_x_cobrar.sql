...
SELECT        A.IdEmpresa, A.IdSucursal, A.IdBodega, 'NTDB' AS CreDeb, CASE WHEN A.NumNota_Impresa IS NULL THEN 'N/D#' + CAST(A.IdNota AS varchar(20)) 
                         ...
                         ROUND(SUM(B.sc_total) - ISNULL(SUM(CB.dc_ValorPago), 0), 2) AS Saldo, ISNULL(SUM(CB.dc_ValorPago), 0) AS totalCobrado, Bo.bo_Descripcion, ROUND(SUM(B.sc_subtotal), 2) AS sc_subtotal, ROUND(SUM(B.sc_iva), 2) 
                         AS sc_iva, A.no_fecha_venc, isnull(vwcxc_cobros_x_vta_nota_x_RetFuente_Sumatoria.dc_ValorPago, 0) AS RtFT, isnull(vwcxc_cobros_x_vta_nota_x_RetIVA_Sumatoria.dc_ValorPago, 0) AS RtIVA, Cli.Codigo AS CodCliente, 
                         tb_persona.pe_nombreCompleto, tb_empresa.em_nombre, A.Estado, ROUND(SUM(B.sc_total), 2), null
