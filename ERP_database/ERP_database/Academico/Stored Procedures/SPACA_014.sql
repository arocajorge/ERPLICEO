...
SELECT mc.IdEmpresa, mc.IdMatricula, mc.IdMateria, alu.Codigo, p.pe_nombreCompleto, m.IdAnio, m.IdSede, m.IdNivel, m.IdJornada, m.IdCurso, m.IdParalelo, m.IdAlumno, AN.Descripcion, sn.NomSede, sn.NomNivel, sn.OrdenNivel, nj.NomJornada, 
                  ...
                  mc.CalificacionP5, mc.CalificacionP6, mc.PromedioQ2, mc.ExamenQ2, mc.PromedioFinalQ2, round(cast(((isnull(mc.PromedioFinalQ1,0)+isnull(mc.PromedioFinalQ2,0))/2) as float),2) as PromedioQuimestralFinal, mc.ExamenMejoramiento, mc.CampoMejoramiento, mc.ExamenSupletorio, mc.ExamenRemedial, mc.ExamenGracia, mc.PromedioFinal, 
                  mc.IdEquivalenciaPromedio, ep.Codigo AS EquivalenciaPromedioFinal, dbo.aca_AnioLectivo_Curso_Materia.NomMateria, dbo.aca_AnioLectivo_Curso_Materia.NomMateriaArea, dbo.aca_AnioLectivo_Curso_Materia.NomMateriaGrupo, 
...
FROM     dbo.aca_MatriculaCalificacion AS mc INNER JOIN
                  ...
                  dbo.aca_AnioLectivoEquivalenciaPromedio AS ep ON m.IdAnio = ep.IdAnio AND mc.IdEmpresa = ep.IdEmpresa AND mc.IdEquivalenciaPromedio = ep.IdEquivalenciaPromedio LEFT OUTER JOIN
