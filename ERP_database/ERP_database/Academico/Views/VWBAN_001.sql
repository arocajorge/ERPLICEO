CREATE VIEW Academico.VWBAN_001
AS
...
FROM     dbo.ba_tipo_nota WITH (nolock) RIGHT OUTER JOIN
                  dbo.ba_Cbte_Ban_tipo WITH (nolock) INNER JOIN
                  ...
                  dbo.ct_cbtecble_det WITH (nolock) INNER JOIN
                  dbo.ct_plancta WITH (nolock) ON dbo.ct_cbtecble_det.IdEmpresa = dbo.ct_plancta.IdEmpresa AND dbo.ct_cbtecble_det.IdCtaCble = dbo.ct_plancta.IdCtaCble ON dbo.ba_Cbte_Ban.IdEmpresa = dbo.ct_cbtecble_det.IdEmpresa AND 
                  ...
                  dbo.seg_usuario WITH (nolock) ON dbo.ba_Cbte_Ban.IdUsuario = dbo.seg_usuario.IdUsuario LEFT OUTER JOIN
                  dbo.tb_sucursal WITH (nolock) ON dbo.ba_Cbte_Ban.IdEmpresa = dbo.tb_sucursal.IdEmpresa AND dbo.ba_Cbte_Ban.IdSucursal = dbo.tb_sucursal.IdSucursal
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 2, @level0type = N'SCHEMA', @level0name = N'Academico', @level1type = N'VIEW', @level1name = N'VWBAN_001';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPane2', @value = N'     Right = 292
            End
            DisplayFlags = 280
            TopColumn = 0
         End
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'Academico', @level1type = N'VIEW', @level1name = N'VWBAN_001';
