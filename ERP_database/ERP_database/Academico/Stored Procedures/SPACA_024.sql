...
FROM     dbo.aca_MatriculaCalificacionParcial AS mp INNER JOIN
                  dbo.aca_Matricula AS m ON mp.IdEmpresa = m.IdEmpresa AND mp.IdMatricula = m.IdMatricula INNER JOIN
				  ...
				  inner join aca_AnioLectivo al on al.IdEmpresa=m.IdEmpresa and al.IdAnio = m.IdAnio
				  left outer join aca_Catalogo cat on cat.IdCatalogo = mp.IdCatalogoParcial
LEFT OUTER JOIN
				  ...
				  RIGHT OUTER JOIN
                  aca_AnioLectivo_Curso_Materia AS cm ON cm.IdEmpresa = m.IdEmpresa AND cm.IdAnio = m.IdAnio AND cm.IdSede = m.IdSede AND cm.IdNivel = m.IdNivel AND 
