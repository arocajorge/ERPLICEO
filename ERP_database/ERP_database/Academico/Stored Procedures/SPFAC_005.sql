...
select a.IdEmpresa, a.IdSucursal, a.IdBodega, a.IdNota, a.no_fecha, a.CreDeb, a.IdTipoNota, a.NaturalezaNota, a.Estado, case when a.NaturalezaNota = 'SRI' THEN a.NumNota_Impresa else a.CodNota end as NumeroNota,
B.SubtotalConDscto, B.ValorIVA, B.Total, isnull(h.Valor_Aplicado,0) Valor_Aplicado, B.Total - isnull(h.Valor_Aplicado,0) Saldo, d.pe_nombreCompleto as NombreCliente, f.pe_nombreCompleto as NombreAlumno, a.IdTipoNota, g.No_Descripcion
...
(
	...
	fa_notaCreDeb_x_fa_factura_NotaDeb as a2 on a1.IdEmpresa = a2.IdEmpresa_nt and a1.IdSucursal = a2.IdSucursal_nt and a1.IdBodega = a2.IdBodega_nt and a1.IdNota = a2.IdNota_nt
	...
	and cast(a2.fecha_cruce as date) <= @FechaHasta
	group by a1.IdEmpresa, a1.IdSucursal, a1.IdBodega, a1.IdNota
) AS h on a.IdEmpresa = h.IdEmpresa and a.IdSucursal = h.IdSucursal and a.IdBodega = h.IdBodega and a.IdNota = h.IdNota
where a.IdEmpresa = @IdEmpresa and a.no_fecha between @FechaDesde and @FechaHasta and a.CreDeb = @CreDeb and a.NaturalezaNota like ('%'+isnull(@Naturaleza,'')+'%') AND A.Estado = 'A'
