...
SELECT a.IdEmpresa, a.IdSucursal, a.IdBodega, a.IdCbteVta, a.vt_tipoDoc, b.Total, ISNULL(c.dc_ValorPago, 0) AS dc_ValorPago, dbo.BankersRounding(b.Total - ISNULL(c.dc_ValorPago, 2), 2) AS Saldo, e.IdAnio, e.IdSede, e.IdJornada, e.IdNivel, 
...
FROM     (SELECT XX.IdEmpresa, XX.IdSucursal, XX.IdBodega_Cbte, XX.IdCbte_vta_nota, XX.dc_TipoDocumento, SUM(XX.dc_ValorPago) AS dc_ValorPago 
                  ...
                  WHERE   (X.cr_estado = 'A') and x.cr_fecha <= @w_FechaCorte
...
WHERE  a.IdEmpresa = @w_IdEmpresa and (a.IdAlumno IS NOT NULL) AND (a.Estado = 'A') AND (dbo.BankersRounding(b.Total - ISNULL(c.dc_ValorPago, 2), 2) > 0) and a.vt_fecha <= @w_FechaCorte
UNION ALL
SELECT a.IdEmpresa, a.IdSucursal, a.IdBodega, a.IdNota, a.CodDocumentoTipo, b.Total, ISNULL(c.dc_ValorPago, 0) AS dc_ValorPago, dbo.BankersRounding(b.Total - ISNULL(c.dc_ValorPago, 2), 2) AS Saldo, null IdAnio, null IdSede, null IdJornada, null IdNivel, 
...
WHERE  a.IdEmpresa = @w_IdEmpresa and (a.IdAlumno IS NOT NULL) AND (a.Estado = 'A') AND (dbo.BankersRounding(b.Total - ISNULL(c.dc_ValorPago, 2), 2) > 0) and a.CreDeb = 'D' and a.no_fecha <= @w_FechaCorte