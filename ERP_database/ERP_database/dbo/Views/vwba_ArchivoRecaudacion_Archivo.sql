...
FROM     dbo.ba_ArchivoRecaudacion WITH (nolock) INNER JOIN
                  dbo.ba_ArchivoRecaudacionDet WITH (nolock) ON dbo.ba_ArchivoRecaudacion.IdEmpresa = dbo.ba_ArchivoRecaudacionDet.IdEmpresa AND 
                  ...
                  dbo.tb_banco WITH (nolock) ON dbo.ba_Banco_Cuenta.IdBanco_Financiero = dbo.tb_banco.IdBanco LEFT OUTER JOIN
                  dbo.tb_persona WITH (nolock) INNER JOIN
                  dbo.aca_Alumno WITH (nolock) ON dbo.tb_persona.IdPersona = dbo.aca_Alumno.IdPersona ON dbo.ba_ArchivoRecaudacionDet.IdEmpresa = dbo.aca_Alumno.IdEmpresa AND 
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPane2', @value = N'     Width = 1776
         Width = 1200
         Width = 1596
         Width = 1200
         Width = 1200
