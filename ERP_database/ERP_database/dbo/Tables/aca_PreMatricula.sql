CREATE TABLE [dbo].[aca_PreMatricula] (
    [IdEmpresa]             INT             NOT NULL,
    [IdPreMatricula]        NUMERIC (18)    NOT NULL,
    [IdAdmision]            NUMERIC (18)    NOT NULL,
    [Codigo]                VARCHAR (200)   NULL,
    [IdAlumno]              NUMERIC (18)    NOT NULL,
    [IdAnio]                INT             NOT NULL,
    [IdSede]                INT             NOT NULL,
    [IdNivel]               INT             NOT NULL,
    [IdJornada]             INT             NOT NULL,
    [IdCurso]               INT             NOT NULL,
    [IdParalelo]            INT             NOT NULL,
    [IdPersonaF]            NUMERIC (18)    NOT NULL,
    [IdPersonaR]            NUMERIC (18)    NOT NULL,
    [IdPlantilla]           INT             NOT NULL,
    [IdMecanismo]           NUMERIC (18)    NOT NULL,
    [IdCatalogoESTPREMAT]   INT             NULL,
    [Fecha]                 DATE            NOT NULL,
    [Observacion]           VARCHAR (MAX)   NULL,
    [IdPuntoVta]            INT             NOT NULL,
    [IdSucursal]            INT             NOT NULL,
    [Valor]                 NUMERIC (18, 2) NOT NULL,
    [ValorProntoPago]       NUMERIC (18, 2) NOT NULL,
    [IdUsuarioCreacion]     VARCHAR (200)   NULL,
    [FechaCreacion]         DATETIME        NULL,
    [IdUsuarioModificacion] VARCHAR (200)   NULL,
    [FechaModificacion]     DATETIME        NULL,
    [IdUsuarioAnulacion]    VARCHAR (200)   NULL,
    [FechaAnulacion]        DATETIME        NULL,
    [MotivoAnulacion]       VARCHAR (MAX)   NULL,
    [IdEmpresa_rol]         INT             NULL,
    [IdEmpleado]            NUMERIC (18)    NULL,
    [EsPatrocinado]         BIT             NULL,
    CONSTRAINT [PK_aca_PreMatricula] PRIMARY KEY CLUSTERED ([IdEmpresa] ASC, [IdPreMatricula] ASC, [IdAdmision] ASC),
    CONSTRAINT [FK_aca_PreMatricula_aca_Admision] FOREIGN KEY ([IdEmpresa], [IdAdmision]) REFERENCES [dbo].[aca_Admision] ([IdEmpresa], [IdAdmision]),
    CONSTRAINT [FK_aca_PreMatricula_aca_Alumno] FOREIGN KEY ([IdEmpresa], [IdAlumno]) REFERENCES [dbo].[aca_Alumno] ([IdEmpresa], [IdAlumno]),
    CONSTRAINT [FK_aca_PreMatricula_aca_AnioLectivo_Curso_Paralelo] FOREIGN KEY ([IdEmpresa], [IdAnio], [IdSede], [IdNivel], [IdJornada], [IdCurso], [IdParalelo]) REFERENCES [dbo].[aca_AnioLectivo_Curso_Paralelo] ([IdEmpresa], [IdAnio], [IdSede], [IdNivel], [IdJornada], [IdCurso], [IdParalelo]),
    CONSTRAINT [FK_aca_PreMatricula_aca_MecanismoDePago] FOREIGN KEY ([IdEmpresa], [IdMecanismo]) REFERENCES [dbo].[aca_MecanismoDePago] ([IdEmpresa], [IdMecanismo]),
    CONSTRAINT [FK_aca_PreMatricula_aca_Plantilla] FOREIGN KEY ([IdEmpresa], [IdAnio], [IdPlantilla]) REFERENCES [dbo].[aca_Plantilla] ([IdEmpresa], [IdAnio], [IdPlantilla]),
    CONSTRAINT [FK_aca_PreMatricula_fa_PuntoVta] FOREIGN KEY ([IdEmpresa], [IdSucursal], [IdPuntoVta]) REFERENCES [dbo].[fa_PuntoVta] ([IdEmpresa], [IdSucursal], [IdPuntoVta]),
    CONSTRAINT [FK_aca_PreMatricula_ro_empleado] FOREIGN KEY ([IdEmpresa], [IdEmpleado]) REFERENCES [dbo].[ro_empleado] ([IdEmpresa], [IdEmpleado]),
    CONSTRAINT [FK_aca_PreMatricula_tb_persona] FOREIGN KEY ([IdPersonaF]) REFERENCES [dbo].[tb_persona] ([IdPersona]),
    CONSTRAINT [FK_aca_PreMatricula_tb_persona1] FOREIGN KEY ([IdPersonaR]) REFERENCES [dbo].[tb_persona] ([IdPersona]),
    CONSTRAINT [FK_aca_PreMatricula_tb_sucursal] FOREIGN KEY ([IdEmpresa], [IdSucursal]) REFERENCES [dbo].[tb_sucursal] ([IdEmpresa], [IdSucursal])
);



